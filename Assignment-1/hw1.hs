------------------------------------------------------------------------
-- Homework 1 CIS 352, Spring 2019, Version 1 
-- Authors: Mo Syed 
------------------------------------------------------------------------
import Data.Char       
...
disEmvowel str = [s | s <- str,  not (isVowel s)  ]
         --  | elem s ['a','e','i','o','u'] = "Nothing" 
      --     | otherwise = s : disEmvowel str
...
smash :: String -> String
smash str = [toLower(s) | s <- str,  Data.Char.isAlpha(s)  ]
...
shift n "" = ""
shift n str = [ toChar(x) | x <- (getnewnum (smash(str)) n)]


--helper function
getnewnum :: String -> Int -> [Int]
getnewnum str n = [ mod (toNum(s)+n) 26 | s <- str]
...
title :: [String] -> [String]
title [] = []
title (first:rest) = [capitalized(first)] ++ [if length(r) >4 then capitalized(r) else r | r <- rest]

--title (first:rest) = [capitalized(first)] ++ if (r <- rest, length(r) <= 4) then r
--                                           else if (r <- rest, length(r) > 4) then (capitalized(r))
--                                         else [] 
...
--restt [] = []
--restt (x:xs) =
 --     if (length (x) <= 4) then map toLower (x) : restt xs
  --    else if ( length(x) >4) then  (toUpper (head x) : map toLower (tail x)) : restt xs
    --  else []

--f1 :: String -> [String]
--f1 [] = []
--f1 (x:xs) =  [[(toUpper (x))] ++ map toLower (xs)]
